# Desplegar Azure Iot Hub

## Crear Grupo de Recursos

az group create `
  --name Industry40 `
  --location eastus2

## Desplegar Bicep con Azure Iot Hub

az deployment group create `
  --resource-group Industry40 `
  --template-file iothub.bicep

## Verificar despliegue

az iot hub show `
  --name fabrica `
  --resource-group Industry40 `
  --output table

## Crear Dispositivo en Azure IoT Hub

az iot hub device-identity create `
  --hub-name fabrica.azure-devices.net `
  --device-id sensor-temperatura-01

az iot hub device-identity show `
  --hub-name fabrica.azure-devices.net `
  --device-id sensor-temperatura-01

## Obtener la cadena de conexión del sensor

az iot hub device-identity connection-string show `
  --hub-name fabrica.azure-devices.net `
  --device-id sensor-temperatura-01

## Copiar la cadena de conexión en el fichero .env

## Crear entorno virtual de Python

python -m venv venv

.\venv\Scripts\Activate.ps1

Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser

## Instalar dependencias

pip install -r requirements.txt

## Ejecutar sensor

python temperature_sensor.py

## Verificar que el sensor funciona:

az iot hub monitor-events `
  --hub-name fabrica.azure-devices.net `
  --device-id sensor-temperatura-01


## Crear Azure Data Explorer cluster

### Compilar

az bicep build --file main.bicep

### Validar antes de desplegar

az deployment group validate `
  --resource-group Industry40 `
  --template-file main.bicep `
  --parameters `
      iotHubName=fabrica `
      iotHubResourceGroup=Industry40

az deployment group validate `
  --resource-group Industry40 `
  --template-file main.bicep `
  --parameters `
    iotHubName=fabrica `
    iotHubResourceGroup=Industry40 `
  --verbose

### Desplegar

az deployment group create `
  --resource-group Industry40 `
  --template-file main.bicep `
  --parameters `
      clusterName=adx-fabrica `
      databaseName=iotdb `
      iotHubName=fabrica `
      iotHubResourceGroup=Industry40 `
      location=eastus2


Falla:

Ver que recurso es:

az deployment operation group list `
  --resource-group Industry40 `
  --name main `
  --query "[?properties.provisioningState=='Failed'].properties.statusMessage"

az deployment operation group list `
  --resource-group Industry40 `
  --name main `
  -o table


Despliego sin data conection (pego aqui lo que borro)

resource dataConnection 'Microsoft.Kusto/clusters/databases/dataConnections@2024-04-13' = {
  parent: adxDatabase
  name: dataConnectionName
  location: location

  kind: 'IotHub'

  properties: {
    iotHubResourceId: iotHub.id
    consumerGroup: consumerGroup
    sharedAccessPolicyName: 'service'
    tableName: 'Telemetry'
    mappingRuleName: 'TelemetryJsonMapping'
    dataFormat: 'JSON'
  }
}


### La identidad administrada del clúster ADX necesita al menos: IoT Hub Data Reader

#### Obtener Identidad del Cluster ADX

az kusto cluster show `
  --name adx-fabrica `
  --resource-group Industry40 `
  --query identity.principalId


#### Asignar el Rol de la Identidad del Cluster al Grupo de Recursos donde está Azure Iot Hub

az role assignment create \
  --assignee <principalId> \
  --role "IoT Hub Data Reader" \
  --scope <iotHubResourceId>

### Crear tabla en adx

.create table Telemetry (
    deviceId:string,
    temperature:real,
    humidity:real,
    timestamp:datetime
)

### Crear el Mapping JSON

.create table Telemetry ingestion json mapping "TelemetryJsonMapping"
'[
  {"column":"deviceId","path":"$.deviceId"},
  {"column":"temperature","path":"$.temperature"},
  {"column":"humidity","path":"$.humidity"},
  {"column":"timestamp","path":"$.timestamp"}
]'


## Borrar grupo de recursos

az group delete `
  --name Industry40 `
  --yes `
  --no-wait

## Desactivar entorno virtual Python

deactivate

### Otros comandos de la CLI de Azure útiles

az iot hub device-identity list \
  --hub-name fabrica.azure-devices.net

az iot hub device-identity delete \
  --hub-name fabrica.azure-devices.net \
  --device-id temp-sensor-01

